...
// sends, so new appliances are data (anatomy.py), not code — same principle
// as the GPU app's AnatomyView. The `active` set lights regions up during
// the power-on trace playback.
...
const KIND_STYLE: Record<RegionKind, { fill: string; stroke: string; text: string }> = {
  ...
  nvram: { fill: "#1c1f3f", stroke: "#5a4fc9", text: "#8f7fff" },
...
export function ChassisView({
  ...
  return (
    ...
    >
      ...
      {/* Orientation: drive bay / bezel end vs node canister / cabling end. */}
      <text
        x={MARGIN}
        y={H + 2.6}
        fill="#5a6b82"
