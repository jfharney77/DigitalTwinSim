"""Geometry/data invariants for the PowerStore enclosure floorplan."""
...
def test_node_ab_symmetry():
    """Every per-node region has a same-kind counterpart on the other node."""
...
def test_photos_are_local_product_images():
    photos = [ANATOMY.photo] + [r.photo for r in ANATOMY.regions]
    for p in photos:
        if p is not None:
            assert p.url.startswith("/powerstore") and p.url.endswith(".webp"), p.url
