...
export const KIND_LABEL: Record<RegionKind, string> = {
  gpu: "Blackwell GPUs",
  compute: "Grace CPUs",
  network: "scale-out NIC / DPU",
  nvswitch: "NVLink switch trays",
...
export function AnatomyPage() {
  ...
  // Legend shows only the kinds this rack actually has.
  const kinds = anatomy
        ...
        {anatomy && (
          <div className="an-card">
            ...
            <div className="mini an-hint">
              ...
              layout is a stylized front-of-rack elevation: power shelves and
              rack management at the top, identical GB200 compute trays above
              and below the NVLink switch trays at mid-rack (kept central so
              every copper run stays short), and the liquid loop — CDU below,
              manifolds up the right edge — reaching every tray. A real rack
              carries 18 compute trays and 9 switch trays; four and two are
              drawn here to keep the pattern readable.
            </div>
          </div>
        )}
        {hovered && hover && (
