...
export async function fetchAnatomy(product: string): Promise<SystemMap> {
...
export async function fetchRedfish(
  state: SimState,
  product: string,
): Promise<object> {
  const r = await fetch(`${BASE}/redfish/thermal?product=${product}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(state),
  });
  if (!r.ok) throw new Error(`redfish ${r.status}`);
  return r.json();
}
