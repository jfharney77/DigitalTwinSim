import type { ChassisMap, SimState, ThermalRegion } from "../types";

// The XR sled, painted as a thermal map: regions colored by temperature
// on a fixed 20–110 °C scale (color-blind-safe blue→yellow→red ramp),
// with airflow arrows whose speed class follows fan rpm. Clicking a fan
// toggles a kill switch; clicking the filter changes it (fouling → 0);
...
// Blue → teal → yellow → orange → red; distinguishable under CVD because
// lightness rises monotonically toward yellow then falls with red kept
// dark. Fixed scale.
...
export function ThermalChassisView({
  anatomy,
  state,
  deadFans,
  selected,
  ...
  const rx = (r: ThermalRegion) => r.x + MARGIN;
  const ry = (r: ThermalRegion) => r.y + MARGIN;
  const rpm = state?.fanRpmPct ?? 0;
  const fouling = state?.foulingPct ?? 0;
  // Airflow arrow dash animation speed classes by rpm.
  ...
  return (
    ...
    >
      ...
      />
      {/* Airflow arrows under the blocks: front (left) to rear (right). */}
      {state?.poweredOn && (
        <g
          className={flowClass}
          stroke={tempColor(state.inletEffectiveC)}
          strokeWidth={0.5}
          strokeDasharray="2.5 2"
          opacity={0.8}
        >
          {[8, 23, 38].map((y) => (
            <line key={y} x1={3} y1={y} x2={W - 3} y2={y} />
          ))}
        </g>
      )}
      {anatomy.regions.map((r) => {
        ...
        const isFilter = r.kind === "filter";
        const fanIdx = isFan ? Number(r.id.split("-")[1]) : -1;
        const dead = isFan && deadFans.has(fanIdx);
        // The filter darkens with its fouling — dust is not hot, just there.
        const fill = dead
          ? "#20242a"
          : isFilter
            ? lerpColor("#3a4a5e", "#181c22", Math.min(1, fouling / 50))
            : tempColor(temp);
        ...
        return (
          <g
            ...
            onClick={(e) => {
              ...
              else if (isFilter && onCleanFilter && fouling > 0) onCleanFilter();
          ...
          >
            <rect
              ...
              fill={fill}
              stroke={isSel ? "var(--accent)" : dead ? "#c8281e" : "#0d1420"}
              strokeWidth={isSel ? 0.6 : 0.3}
              opacity={dead ? 0.9 : 1}
            />
            {(showLabel || dead) && !showVLabel && (
              <text
                x={rx(r) + r.w / 2}
                y={ry(r) + (r.h < 6 ? r.h / 2 + hSize * 0.35 : 2.4)}
                textAnchor="middle"
                fill={dead ? "#e07b6a" : isFilter ? "#8fa3bd" : "#0d1420"}
            ...
            {/* Fouling readout on the filter strip. */}
            {isFilter && state && fouling > 0 && (
              <text
                x={rx(r) + r.w / 2}
                y={ry(r) + r.h - 1.6}
                textAnchor="middle"
                ...
                fontSize={1.5}
                fontWeight={700}
                transform={`rotate(-90 ${rx(r) + r.w / 2} ${ry(r) + r.h - 1.6})`}
              >
                {fouling.toFixed(0)}%
              </text>
            )}
            {/* Temperature readout on the larger blocks. */}
            {r.w >= 10 && r.h >= 8 && state && !dead && !isFilter && (
              <text
                x={rx(r) + r.w / 2}
                y={ry(r) + r.h - 1.6}
                textAnchor="middle"
