...
import type { CatalogCategory, DeviceAnatomy } from "../types";
  ...
  return (
    <>
      <div className="an-hero">
        ...
        <p>
          Configuring for on-device inference inverts the usual order. The
          processor is not the first decision — the accelerator card is,
          because the memory it brings decides which models will fit at
          all. Everything after it follows from that one number. Hover a
          category to light up where its parts live on the device.
        </p>
      </div>
            ...
            <div className="mini">
              {active
                ? active.regionIds.length > 0
                  ? `${active.name} — highlighted on the device map.`
                  : `${active.name} — not a drawn part of the device map (software, services, or deployment).`
                : "Hover a category to highlight its home on the device map."}
            </div>
          </section>
        )}
