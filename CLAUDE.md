...
GPU/cuda/      twinprobe.cuh, twin-sampler, twin-run, twininject/, lessons/00..07_*.cu, Makefile
...
- **The wider fleet (2026-08):** beyond the 4060, `profiles.py` carries real dies as data — `H100-SXM` (132 SMs), `B300-Blackwell-Ultra` (160), `RTX-5090` (170), `MI300X` (304 CUs). SM/CU counts match the shipping parts; lanes per SM are *representative* (scope guardrail — the UI is in per-SM dense mode anyway) and the bandwidth ratios order the ridge points like the real parts' FP32/byte figures (5090 > 4060 > MI300X > H100 > B300, pinned in `test_profiles.py`). `anatomy.py` has eight dies (GH100, GA100, AD102, Navi 31, GB200, plus GB300 Blackwell Ultra, GB202/RTX 5090, MI300X), all with five-level overviews. CUDA lesson 07 (`07_bigger_dies.cu`, "the die is a parameter") sizes its grid from `multiProcessorCount`; its guided-tour steps replay representative recordings of the same binary on H100 (132 SMs) and B300 (160 SMs) geometry — `test_smallwins3.py` pins `toursAvailable == 8`.
- GPU is the only component with a spec series past 06: **`spec_07` through `spec_21`** drive the RTX 4060 profile, CUDA live co-browse, the lesson curriculum, per-SM Gantt, diff mode, device-agnostic sessions, mid-kernel streaming, measured roofline, grid sampling, CUPTI injection, lesson tours, and three rounds of small wins. Read the spec before changing anything under `live*.py`, `tour.py`, or `cuda/` — several of them encode invariants the tests enforce. **`spec_22`–`spec_31` are built** (2026-08 review round, in dependency order 22→24→26→23→25→27→28→30→29→31): rectangular M×K×N matmul (`m`/`k_dim`, 0 = square), occupancy model (`block_size`, `Summary.occupancy`, limiter), `llm_decode` workload (`app/llm.py`, KV-cache intensity fall + regime flip pinned), tensor-core mode (`TensorSpec`, `execution="tensor"`, `CoreState "mma"`, `DTYPE_BITS`/fp8/fp4), power model (`SimState.powerWatts`, exact-closure energy ledger, `peak_power_w` calibration), two-GPU NVLink scale-up (`gpus=2`, `exchange` phase, per-profile `Link`), fleet replay (`?asProfile=` remap with structural `placement: measured|modeled` provenance), cross-tab atlas (`die_id` on profiles, `GET /api/atlas` — the snapshot moved 23→24 routes, `#anatomy/<a>/vs/<b>`), reading levels everywhere (registry 8→63 blocks: tour + all region descriptions), and spec_31's hardware-campaign tooling (`make verify-hardware`, `cuda/verify_hardware.py`, `cuda/promote_recording.py` — the capture campaign itself still awaits nvcc). Each spec file carries an appended "Implementation notes" section recording deviations; the suite is at 490 tests.
...
- **Honest caveat that outlives this note:** `nvcc` has never run on this machine — `twinprobe.cuh`, the lessons, and `cuda/twininject/` have never been compiled. The header↔backend wire contract is what's CI-tested (probe-sample fixtures). First hardware run may need compile fixes; toolkit install is step 0 of `GPU/cuda/README.md`. The first-hardware-run campaign is now a written protocol — `GPU/spec_31_hardware_provenance.md`, rerunnable as `make verify-hardware` in `GPU/cuda/`, with promotion tooling (`promote_recording.py`) and the README's "First hardware run" section — so campaign day is turnkey; the caveat stands until that campaign actually runs.
...
## DellNativeEdge — edge-orchestration digital twin (twenty-fourth component)

Same architecture, applied to **Dell NativeEdge** — Dell's edge operations software platform (2023; 2.0 in 2024; basis of Dell Distributed Private Cloud), managing estates of servers, gateways, workstations, and desktops deployed outside any datacenter. Built post-loop from `DellNativeEdge/initial_spec.md` (iteration 1's spec). Like CloudIQ, the subject is **software**, so both metaphors adapt: the "anatomy" is a platform architecture diagram and the "power-on trace" is the **zero-touch onboarding of one site** — sealed crate to managed estate. The one idea: **nobody touches the device** — the device wakes, proves cryptographically that it is the machine Dell built, and asks the Orchestrator what to become; **`operatorActions` exists to be 1** (power and a network cable) and never increments again. See `DellNativeEdge/README.md`.

```
DellNativeEdge/backend/   app/{models,anatomy,engine,catalog,usecases,leveling,main}.py + tests/
DellNativeEdge/frontend/  src/{api,types,level}.ts, App.tsx, components/{PlatformView,ArchitecturePage,CatalogPage,UseCasePage,OnboardControls,OnboardCounters,LevelControl}.tsx
DellNativeEdge/scripts/   start_backend.sh, start_frontend.sh, start_all.sh, stop_all.sh
```

- Run: `./DellNativeEdge/scripts/start_all.sh` (backend :8014 background, frontend :5187 foreground — the ports the spec reserved). Stop: `./DellNativeEdge/scripts/stop_all.sh`.
- Backend tests: `cd DellNativeEdge/backend && . .venv/bin/activate && python -m pytest -q`
- Frontend typecheck/build: `cd DellNativeEdge/frontend && npm run build`
- Vite proxies `/api` → `http://localhost:8014`. Trace endpoint is `GET /api/onboard` returning `OnboardResponse`; pages hash to `#architecture` / `#capabilities` / `#usecases` (CloudIQ style).

Key points beyond the CloudIQ pattern:

- **Estate model shapes** (`models.py`): `PlatformMap` / `PlatformRegion` / **`OnboardState`**. `RegionKind` is estate-specific: `endpoint · network · identity · orchestrator · blueprint · catalog · policy · observability`. `OnboardState` carries `endpointsOnline` (0 → 4), **`operatorActions` (0 → 1, then frozen — the hero counter)**, `trustEstablished` (bool, monotone), and `progressPercent` (0 → 100).
- **Onboarding trace** (`engine.py`, pure — AST-checked): phase order `crated→power→attest→onboard→provision→blueprint→workload→managed` never regresses. Signature invariants (`test_engine.py`): **exactly one human action** (`operatorActions` 0 before `power`, 1 there, 1 forever); **nothing runs before trust is established** (no endpoint online, no workload phase reached — zero-touch without attestation is just an unauthenticated machine on your network); **trust is never revoked mid-sequence**; **the Orchestrator is never the thing being onboarded** (excluded from `endpointsOnline`); **the estate scales in lockstep** (endpoints light as a set); **attestation is the longest stage** (unique max `cycleCost` — the security step dwelt on, not skipped); the final step lights the entire platform (managed means the loop is closed).
- **Geometry carries the lesson** (`anatomy.py`, stylized 100×58 left→right): `test_the_endpoint_band_is_uniform` pins ≥4 identical endpoint regions (an estate is one building block repeated); `test_the_orchestrator_is_central_and_singular` pins exactly one orchestrator, drawn larger than everything and horizontally central; `test_the_estate_is_left_and_the_control_is_right` pins endpoints/WAN/identity strictly left of the Orchestrator and blueprints/catalog/policy/observability strictly right — the axis is the direction of trust. Only visual is the credited schematic `frontend/public/nativeedge-platform.svg`.
- **Catalog (10 categories) and use cases (3) are backend data**: endpoint hardware (PowerEdge XR / gateways / Precision workstations), device identity & secure onboarding, the Orchestrator (HA), blueprints, application catalog (Dell/ISV + customer workloads), Zero Trust at the edge, connectivity (intermittent WAN / private 5G), observability & AIOps, edge AI inference, services & validated designs. Use cases: 400 retail branches provisioned by shop managers, a vision model on the line updated without a visit, and a distributed private cloud across a utility's substations. `test_catalog.py` enforces resolvable ids as elsewhere.
- Copy spells out edge-operations vocabulary (zero-touch provisioning, attestation, blueprint, ISV, OT, private 5G, "day 2") on first use; the anatomy overview **and all 8 trace steps** are authored at all five reading levels. Counts/timings illustrative. Referenced by `CustomerSetup/McLarenRacing/` as the trackside edge-management block (representative). Cross-references kept intact: ProMaxPlus (one endpoint of this estate), CloudIQ (the watching half), iDRAC (the same idea, one machine at a time), FortZero (the Zero Trust argument, applied per endpoint).

## DellPowerEdgeR760Thermal — interactive power & thermal simulator (twenty-fifth component)

The R760's **second twin**: same machine as `DellPowerEdgeR760/`, the other question — not what happens when it turns on but what happens while it runs. An interactive, simplified physics model of **configuration → load → power → heat → fan response → feedback**, built from the user-supplied spec at `DellPowerEdgeR760/r760-interactive-simulator-spec.md` (its §11 records the repo adaptations). Explicitly not CFD and not telemetry-fed: correct relationships and orders of magnitude, with every constant in `backend/app/constants.py` carrying units + a `source` field and estimates flagged through to the UI.

```
DellPowerEdgeR760Thermal/backend/   app/{models,constants,engine,validation,anatomy,presets,leveling,main}.py + tests/
DellPowerEdgeR760Thermal/frontend/  src/{api,types,level}.ts, App.tsx, components/{ThermalChassisView,BuildPanel,Instruments,StripCharts,LevelControl}.tsx
DellPowerEdgeR760Thermal/scripts/   start_backend.sh, start_frontend.sh, start_all.sh, stop_all.sh
```

- Run: `./DellPowerEdgeR760Thermal/scripts/start_all.sh` (backend :8030 background, frontend :5203 foreground). Stop: `./DellPowerEdgeR760Thermal/scripts/stop_all.sh`.
- Backend tests: `cd DellPowerEdgeR760Thermal/backend && . .venv/bin/activate && python -m pytest -q`
- Frontend typecheck/build: `cd DellPowerEdgeR760Thermal/frontend && npm run build`
- Vite proxies `/api` → `http://localhost:8030`. **The trace is scenario-driven**: `POST /api/simulate` takes a `Scenario` (config, workload dials, environment, timed events) and returns the deterministic trace — the Alienware twin's pattern generalized; `GET /api/simulate` runs the default scenario (CustomerSetup's chip enrichment expects a GET). Other endpoints: `/api/anatomy`, `/api/constants`, `/api/presets/{configs,workloads}`, `/api/scenarios`, `/api/explain`.

Key points beyond the twin pattern:

- **Scenario model shapes** (`models.py`): `Scenario` (`ServerConfig` + `Workload` + `Environment` + `SimEvent[]`) in; `SimResponse` (`Validation[]` + `SimState[]` trace + `LogEntry[]` + `Summary`) out. `SimState` carries the full instrument set per tick — component powers, DC/AC, PSU efficiency point, fan rpm/CFM, zone and component temperatures, throttle state, and `regionTemps` keyed to the chassis map. Interactive mid-run actions (kill a fan, raise the inlet) are **timed events**, keeping the engine pure and every run reproducible.
- **Physics engine** (`engine.py`, pure — AST-checked, `random` also banned): 1 s timestep; CPU power `idle + (TDP−idle)·util^1.4` with 60 s boost; cubic fan law; serial thermal zones with a lane split; first-order thermal masses (τ 20 s silicon / 300 s drives); proportional fan controller; protective behaviors (stepped throttling, overtemp/inlet power-off, PSU overcurrent trip, fan/PSU failures). Two conservation identities are asserted in `test_engine.py`, house style: **power balance every tick** (Σ component powers = DC; AC = DC ÷ η(load) — the fan-feedback loop as asserted fact) and **heat balance at steady state** (ΔT = DC/(ṁ·cp) — the IR7000 identity inside one server). The spec's §9 acceptance criteria are pytest cases (idle envelope, max-config power band, ramp-then-throttle-in-order-with-lag).
- **The chassis map reuses the R760 power-on twin's floorplan geometry** (100×46, front at x=0), repainted by temperature on a fixed 20–110 °C color-blind-safe scale; `test_region_temps_match_anatomy` pins the engine↔map contract and `test_airflow_axis_is_drawn` pins drives-fans-PSUs front-to-back. Clicking a fan kills it (an event at the current cursor); airflow arrows animate with rpm.
- **Validation rules engine** (`validation.py`, spec §6): heatsink ≥250 W and Gold-fans (≥300 W or any double-wide GPU) are errors; max-TDP × hot inlet, PSU oversubscription (warn, don't block — then simulate the trip), and altitude derating are warnings; every rule carries a source; presets must pass their own hard rules (`test_model_data.py`).
...
**Authoring status.** **Every anatomy overview** is authored at all five levels, on all 20 twins (including all eight GPU dies and both Alienware interiors). **Trace steps** are authored on 18 twins — 188 steps, all five levels each — with two exceptions:
