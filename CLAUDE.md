...
**Authoring status.** The control is wired and green on all 19 twins. Prose is authored for **every twin's anatomy overview** (and all five GPU dies and both Alienware interiors), plus the **full nine-step trace** of `DellPrivateCloud/`. Region descriptions, catalog entries, and use-case narratives elsewhere currently fall back — they read the same at every level. Extending them is incremental and safe: wrap the string in `L(...)`, add the variants you have, and the tests will tell you if the scale is inverted or a level is empty.
