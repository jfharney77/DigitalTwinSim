  ...
  return (
    <>
      <div className="an-hero">
        ...
        <p>
          The first category is the architecture itself, because that
          decision determines whether the later ones stay decisions at
          all. On a hyperconverged cluster the hypervisor is not chosen
          from a catalog — it is what the thing is made of. Hover a
          category to light up where it lives in the stack.
        </p>
      </div>
            ...
            <div className="mini">
              {active
                ? active.regionIds.length > 0
                  ? `${active.name} — highlighted in the stack.`
                  : `${active.name} — not tied to a single layer (it spans the stack).`
                : "Hover a category to highlight its layer."}
            </div>
          </section>
        )}
