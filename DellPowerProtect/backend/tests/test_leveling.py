...
from app.models import LifecycleResponse
...
def test_resolution_is_total_at_every_level():
    ...
    for level in LEVELS:
        ...
        trace = leveled(LifecycleResponse(trace=simulate()), level)
...
def test_default_level_is_the_untouched_text():
    ...
    base = LifecycleResponse(trace=simulate())
...
def test_levels_actually_differ_somewhere():
    ...
    assert (
        ...
        leveled(LifecycleResponse(trace=simulate()), 1).model_dump()
        != leveled(LifecycleResponse(trace=simulate()), 5).model_dump()
    )
