...
// The build-to-order menu: every category is a physical slot in the node pair
// or DME (or software that ships with the array). Hovering a category lights
// up where it lives on the mini floorplan.
  ...
  return (
    <>
      <div className="an-hero">
        ...
        <p>
          A PowerMax array is configured to order and scales out in node pairs:
          directors, cache, and I/O modules fill slots you can see on the
          engine floorplan, drives populate the Dynamic Media Enclosures, and
          the software ships as an inclusive package. Hover a category to light
          up where its parts live.
        </p>
      </div>
            ...
            <div className="mini">
              {active
                ? active.regionIds.length > 0
                  ...
                  : `${active.name} — not a physical slot in the engine (software, licensing, or cabinet).`
                : "Hover a category to highlight its home in the node pair."}
            </div>
          </section>
        )}
