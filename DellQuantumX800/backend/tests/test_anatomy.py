"""Geometry/data invariants for the Quantum-X800 fabric map."""
...
def test_tier_symmetry():
    """Every spine is identical to every other spine, and likewise leaves
    and endpoints — a fabric tier is one building block repeated, which is
    what makes path lengths uniform."""
    by_kind = {}
    for r in ANATOMY.regions:
...
def test_tiers_are_vertically_ordered():
    """Spines above leaves above endpoints — the diagram encodes the
    two-hop path, so the geometry is part of the lesson."""
    by_kind = {}
    for r in ANATOMY.regions:
        by_kind.setdefault(r.kind, []).append(r)
...
def test_the_subnet_manager_is_small_and_beside_the_fabric():
    """The twin's own geometry lesson: the SM is the most important thing
    in the fabric's life and the least important thing in a packet's, so
    it must be drawn smaller than every fabric block and strictly beside
    the tree — never atop it, where a reader would assume data flows
    through it."""
    manager = next(r for r in ANATOMY.regions if r.kind == "manager")
    for r in ANATOMY.regions:
        if r.kind != "manager":
            assert manager.w * manager.h < r.w * r.h, (
                f"the SM must be drawn smaller than {r.id}"
            )
    spines = [r for r in ANATOMY.regions if r.kind == "spine"]
    assert manager.x + manager.w <= min(r.x for r in spines), (
        "the SM must sit beside the spine tier, not above or inside it"
    )


def test_every_leaf_has_an_endpoint_rack():
    """Each leaf is top-of-rack for one GPU rack in this drawing, so the
    two tiers come in matched counts."""
...
def test_topology_counts():
    ...
    assert len(kinds["manager"]) == 1, "one brain per fabric"
