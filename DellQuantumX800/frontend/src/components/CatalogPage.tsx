...
import type { CatalogCategory, FabricAnatomy } from "../types";
  ...
  return (
    <>
      <div className="an-hero">
        ...
        <p>
          An InfiniBand fabric is designed against collective traffic
          patterns, not user counts: pick the switch generation, then the
          topology's shape, and the rest follows from the architecture —
          credit-based transport is not an option but the constitution, one
          subnet manager programs every route, and SHARP decides whether the
          switches join the arithmetic. Hover a category to light up where
          its parts live in the topology.
        </p>
      </div>
            ...
            <div className="mini">
              {active
                ? active.regionIds.length > 0
                  ? `${active.name} — highlighted on the topology.`
                  : `${active.name} — not a drawn part of the topology (validated designs or services).`
                : "Hover a category to highlight its home in the topology."}
            </div>
          </section>
        )}
