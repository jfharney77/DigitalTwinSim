...
*several* annotated subjects rather than one — here, the two laptop interiors —
...
def test_every_interior_resolves_at_every_level():
    for level in LEVELS:
...
def test_levels_actually_differ_for_every_interior():
    """each interior carries its own authored prose, so each must respond to the
    control — one leveled interior and a static one would be worse than
    none, because the control would look broken on the other tab."""
    for die_id, die in ANATOMIES.items():
        assert (
