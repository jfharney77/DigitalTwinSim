...
export async function fetchAnatomy(): Promise<PlatformAnatomy> {
  const r = await fetch(url("/anatomy"));
  if (!r.ok) throw new Error(`anatomy ${r.status}`);
  return r.json();
}

export async function fetchDataPath(): Promise<DataResponse> {
  const r = await fetch(url("/datapath"));
  if (!r.ok) throw new Error(`datapath ${r.status}`);
  return r.json();
}
