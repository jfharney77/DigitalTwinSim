"""Invariants for the reading-level system: the mechanism (leveling.py)
is the repo-shared one byte-for-byte; the leveled surfaces here are the
the product-map overviews, the Explain entries, and the guided-scenario
narration."""
...
def test_default_level_is_the_untouched_text():
    for m in MAPS.values():
        assert leveled(m, DEFAULT_LEVEL).model_dump() == m.model_dump()
    assert [
...
def test_levels_actually_differ_somewhere():
    m = MAPS["telecomblocks"]
    assert leveled(m, 1).model_dump() != leveled(m, 5).model_dump()
