...
// cluster (nodes, media, protocols, interconnect) or something the
// architecture depends on (OneFS software, replication, security). Hovering
// a category lights up where it lives on the mini map.
  ...
  return (
    <>
      <div className="an-hero">
        ...
        <p>
          There is no volume-layout category here, because there are no
          volumes to lay out. The first decision is the node tier — how much
          of each node is flash, and how much is capacity — and every later
          decision rides on the same single file system whichever tier holds
          it. Hover a category to light up where its parts live in the
          cluster.
        </p>
      </div>
            ...
            <div className="mini">
              {active
                ? active.regionIds.length > 0
                  ? `${active.name} — highlighted in the cluster.`
                  : `${active.name} — not a drawn part of the cluster map (software, services, or policy).`
