...
import type { Photo, ServerAnatomy, RegionKind } from "../types";
...
export const KIND_LABEL: Record<RegionKind, string> = {
  gpu: "SXM GPUs",
  nvswitch: "NVSwitch complex",
  compute: "x86 host",
  network: "per-GPU NICs",
  storage: "NVMe bay",
  cooling: "fan wall",
  power: "PSU bank",
  management: "iDRAC",
...
export function AnatomyPage() {
  ...
  // Legend shows only the kinds this server actually has.
  const kinds = anatomy
        ...
        {anatomy && (
          <div className="an-card">
            ...
            <div className="mini an-hint">
              ...
              layout is a stylized top-down look into the 6U chassis, front
              of the server at the left: the NVMe bay and fan wall, then the
              HGX baseboard — eight identical SXM GPUs with the NVSwitch
              strip at their right edge, where the NVLink domain ends — and
              at the rear the x86 host, iDRAC, PSU bank, and eight scale-out
              NICs, drawn one row per GPU because each GPU owns exactly one.
              Air flows left to right; the geometry is a mental model, not a
              service manual.
            </div>
          </div>
        )}
