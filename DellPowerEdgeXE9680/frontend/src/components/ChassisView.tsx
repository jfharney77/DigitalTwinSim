import type { ServerAnatomy, ServerRegion, RegionKind } from "../types";

// Data-driven chassis floorplan renderer: draws whatever regions the backend
// sends, so a different server is data (anatomy.py), not code — same
// principle as the GPU app's AnatomyView and the other chassis twins'
// ChassisView. The `active` set lights regions up during power-on playback.
...
const KIND_STYLE: Record<RegionKind, { fill: string; stroke: string; text: string }> = {
  gpu: { fill: "#2b2412", stroke: "#6b5a2b", text: "#c9a94f" },
  nvswitch: { fill: "#1c1f3f", stroke: "#5a4fc9", text: "#8f7fff" },
  compute: { fill: "#241f33", stroke: "#4a4066", text: "#8a7ab5" },
  network: { fill: "#16281a", stroke: "#3a6647", text: "#6ab585" },
  storage: { fill: "#241a2e", stroke: "#5a4080", text: "#a07ac9" },
...
export function ChassisView({
  ...
  const rx = (r: ServerRegion) => r.x + MARGIN;
  const ry = (r: ServerRegion) => r.y + MARGIN;
  ...
  return (
    ...
    >
      ...
      {/* Orientation: top-down into the chassis; air flows left to right. */}
      <text x={MARGIN} y={H + 2.6} fill="#5a6b82" fontSize={1.7} letterSpacing={0.3}>
        FRONT — NVMe bay &amp; fan wall
      </text>
      <text
        x={W - MARGIN}
        y={H + 2.6}
