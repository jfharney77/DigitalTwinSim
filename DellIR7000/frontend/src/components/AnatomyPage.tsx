...
import type { Photo, RackAnatomy, RegionKind } from "../types";
...
export const KIND_LABEL: Record<RegionKind, string> = {
  ...
  manifold: "supply / return manifold",
  coldplate: "IT bay (cold plates)",
  airdoor: "rear-door heat exchanger",
  power: "power shelf / busbar",
  facility: "facility water",
  sensor: "leak / flow sensors",
};

// Swatches for the legend; keep in sync with KIND_STYLE in RackView.
...
export function AnatomyPage() {
  ...
  // Legend shows only the kinds this loop actually has.
  const kinds = anatomy
        ...
        {anatomy && (
          <div className="an-card">
            ...
            <div className="mini an-hint">
              ...
              layout draws the rack as what it thermally is — a loop: the
              CDU at the bottom pumps coolant up the supply manifold on the
              left, through cold plates in every IT bay, and back down the
              return on the right; the rear-door heat exchanger (far right)
              catches the air-side remainder; and everything exits as warm
              facility water. The bays are generic on purpose — to the loop,
              any payload is just heat.
            </div>
          </div>
        )}
        {hovered && hover && (
