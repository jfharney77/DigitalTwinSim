...
// loop or something the loop depends on (the rack frame, coolant chemistry,
// services). Hovering a category lights up where it lives on the mini
// floorplan.
  ...
  return (
    <>
      <div className="an-hero">
        ...
        <p>
          A liquid-cooled rack is designed as a matched system: the power
          envelope sets the heat load, and the CDU, manifolds, cold plates,
          and rear door are sized together to remove exactly that. The
          payload is deliberately the last choice — to the loop, anything
          racked in the bays is just heat. Hover a category to light up
          where its parts live in the loop.
        </p>
      </div>
            ...
            <div className="mini">
              {active
                ? active.regionIds.length > 0
                  ? `${active.name} — highlighted on the loop floorplan.`
                  : `${active.name} — not a drawn part of the loop (rack frame, chemistry, or services).`
                : "Hover a category to highlight its home in the loop."}
            </div>
          </section>
        )}
