import type { RackAnatomy, RackRegion, RegionKind } from "../types";

// Data-driven loop floorplan renderer: draws whatever regions the backend
// sends, so a bigger or different rack is data (anatomy.py), not code —
// same principle as the other twins' ChassisView/RackView. The `active`
// set lights regions up during thermal-trace playback.
...
const KIND_STYLE: Record<RegionKind, { fill: string; stroke: string; text: string }> = {
  cdu: { fill: "#122b2b", stroke: "#2e5c54", text: "#4fa08a" },
  manifold: { fill: "#12233a", stroke: "#3d5a9e", text: "#4f7cff" },
  coldplate: { fill: "#2b2412", stroke: "#6b5a2b", text: "#c9a94f" },
  airdoor: { fill: "#16281a", stroke: "#3a6647", text: "#6ab585" },
  power: { fill: "#2b1a1a", stroke: "#6b3a3a", text: "#c97a6a" },
  facility: { fill: "#1c1f3f", stroke: "#5a4fc9", text: "#8f7fff" },
  sensor: { fill: "#12282e", stroke: "#2e5666", text: "#4fa0c9" },
};
...
export function RackView({
  ...
  return (
    <svg
      viewBox={`0 0 ${W} ${H + 4}`}
      aria-label={`${anatomy.name} loop floorplan`}
    ...
    >
      ...
      {/* Orientation: the coolant's path, left to right and bottom to top. */}
      <text x={MARGIN} y={H + 2.6} fill="#5a6b82" fontSize={1.7} letterSpacing={0.3}>
        SUPPLY UP THE LEFT — RETURN DOWN THE RIGHT
      </text>
      <text
        x={W - MARGIN}
        y={H + 2.6}
